...
function homeMarkup() {
  ...
  const blogCards = posts.slice(0, 3).map((post) => `<article class="card card-body blog-preview-card"><p class="meta">${esc(post.date)}</p><h3><a href="/blog/${esc(post.slug)}/">${esc(post.title)}</a></h3><p>${esc(post.description)}</p><a class="text-link" href="/blog/${esc(post.slug)}/">Read article <span aria-hidden="true">→</span></a></article>`).join("");
