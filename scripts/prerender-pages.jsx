...
function homeMarkup() {
  ...
  const blogCards = posts.slice(0, 3).map((post) => `<article class="card card-body blog-preview-card">${post.featured_image ? `<img class="blog-card-image" src="${esc(post.featured_image)}" alt="${esc(post.featured_image_alt)}" />` : ""}<p class="meta">${esc(post.date)}</p><h3><a href="/blog/${esc(post.slug)}/">${esc(post.title)}</a></h3><p>${esc(post.description)}</p><a class="text-link" href="/blog/${esc(post.slug)}/">Read article <span aria-hidden="true">→</span></a></article>`).join("");
  const blogSection = blogCards ? `<section class="section ivory" id="resources"><div class="container"><div class="section-heading blog-preview-heading"><p class="eyebrow">OliveHorse Resources</p><h2>Helpful Karate Guides for Parents and Students</h2><p class="lede">Practical articles to help you choose the right training programme and get started with confidence.</p></div><div class="blog-preview-grid">${blogCards}</div><div class="blog-preview-action"><a class="btn btn-secondary" href="/blog/">View all resources</a></div></div></section>` : "";
  return `<a class="skip-link" href="#main">Skip to content</a><header class="site-header"><div class="nav-shell"><a class="brand-link" href="/" aria-label="${esc(business.brand.name)}"><span class="brand-copy"><span class="brand-name">${esc(business.brand.name)}</span><span class="brand-tagline">${esc(business.brand.tagline)}</span></span></a><nav class="nav-links" aria-label="Primary navigation"><a class="nav-link" href="#about">About</a><a class="nav-link" href="#programmes">Programmes</a><a class="nav-link" href="#contact">Contact</a><a class="nav-link" href="/blog/">Resources</a></nav><a class="blog-nav-link" href="/blog/" aria-label="Read OliveHorse blog resources"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 3.75h11a2.25 2.25 0 0 1 2.25 2.25v14.25H7.25A2.25 2.25 0 0 1 5 18V3.75Z"/><path d="M8.5 8h6.5M8.5 12h6.5M8.5 16h4"/></svg><span>Blog</span></a></div></header><main id="main"><section class="hero"><div class="container"><p class="eyebrow">${esc(homepage.hero.eyebrow)}</p><h1>${esc(homepage.hero.heading)}</h1><p class="lede">${esc(verifiedHeroDescription)}</p><a class="btn btn-primary" href="#trial-form">${esc(homepage.hero.primaryCTA.label)}</a></div></section><section class="section ivory" id="programmes"><div class="container"><p class="eyebrow">${esc(homepage.programmesPreview.eyebrow)}</p><h2>${esc(homepage.programmesPreview.heading)}</h2><p class="lede">${esc(homepage.programmesPreview.description)}</p><div class="card-grid">${programmeCards}</div></div></section>${blogSection}<section class="section" id="about"><div class="container"><p class="eyebrow">${esc(homepage.about.eyebrow)}</p><h2>${esc(homepage.about.heading)}</h2>${homepage.about.description.map((paragraph) => `<p class="lede">${esc(paragraph)}</p>`).join("")}</div></section><section class="section orange" id="trial-form"><div class="container"><p class="eyebrow">${esc(homepage.trialForm.eyebrow)}</p><h2>${esc(homepage.trialForm.heading)}</h2><p class="lede">${esc(homepage.trialForm.description)}</p><p>Contact OliveHorse Fitness Academy directly to discuss an introductory trial class.</p></div></section><section class="section" id="contact"><div class="container"><p class="eyebrow">${esc(homepage.contact.eyebrow)}</p><h2>${esc(homepage.contact.heading)}</h2><p class="lede">${esc(homepage.contact.description)}</p></div></section></main><footer class="site-footer"><div class="container footer-bottom"><span>© OliveHorse Fitness Academy. All rights reserved.</span><span class="social-links"><a href="/privacy-policy/">Privacy Policy</a><a href="/terms/">Terms and Conditions</a></span></div></footer>`;
}
